...
void handle_threads(int fd) {
  ...
  while (curr) {
    ...
    offset += snprintf(json_buf + offset, JSON_BUF_SIZE - offset,
                       ...
                       curr->id, curr->tickets, curr->pass, curr->state,
                       curr->stride, ss.stack_used, curr->waiting_fd);
...
int main(void) {
  ...
  // Spawn demo workload (Mix of CPU/IO/Sleep)
  extern void cpu_task(void *); // Reuse or redefine?
  // I'll redefine simple ones here for self-containment

  // Create heavy load to verify Stride
  // 3 CPU tasks with different tickets
  // I'll add logic to "create_cpu_task"

  printf("Starting workload...\n");
  // I need valid functions. I'll define them below.
